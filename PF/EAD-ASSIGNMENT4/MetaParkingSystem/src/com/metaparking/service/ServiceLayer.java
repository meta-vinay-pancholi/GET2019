...
public class ServiceLayer {
	DaoLayer daoLayerObject = new DaoLayer();

	public ArrayList<String> validateUserData(String name, String gender,
			String phone, String email, String org, String pass, String conpass)
			throws SQLException {
		...
		if (errorList.size() <= 0) {
			int result = daoLayerObject.registerUser(name, email, org, pass,
					phone, gender);
			if (result == 1) {
				return null;
			}
	...
	public ResultSet authenticateUser(String pass, String email)
			throws SQLException {
		return daoLayerObject.loginUser(email, pass);
	}
	...
	public ArrayList<String> validateVehicleData(String vname, String vtype,
		...
		if (errorList.size() <= 0) {
			int result = daoLayerObject.registerVehicle(vname, vnumber, empid,
					vtype, identy);
			if (result == 1) {
				return null;
			}
	...
	public ArrayList getFriendOfUser(String user) throws SQLException {
		...
		ResultSet rs = daoLayerObject.getFriendOfUser(user);
		while (rs.next()) {
			System.out.println(rs.getInt("Id"));
			user_friend.add(new UserFriendList(rs.getInt("id"), rs
					.getString("full_name"), rs.getString("Organisation"), rs
					.getString("Contact_number"), rs.getString("Email_Id")));
		}
		return user_friend;
	}

	public ArrayList<VehicleDetails> getVehicleDetails(String empId)
			throws SQLException {
		ResultSet rs = daoLayerObject.getVeichleDetails(empId);
		ArrayList<VehicleDetails> employeeVehicleDetails = new ArrayList<VehicleDetails>();
		while (rs.next()) {
			employeeVehicleDetails.add(new VehicleDetails(rs.getInt("id"), rs
					.getString("Vehicle_Number"), rs.getString("Type")));
		}
		return employeeVehicleDetails;
	}

	/**
	 * function is used to validate the vehicle details when data come from the
	 * get method
	 * @param vehcileId
	 * @param vechileType
	 * @return
	...
	/**
	 * function is used to get complete price of vehicle type
	 * 
	 * @param vechileType
	 * @return
	 * @throws SQLException
	 */
	public ArrayList<Price> getPriceOfVehicle(String vechileType)
			throws SQLException {
		ArrayList<Price> priceOfVechile = new ArrayList<Price>();
		ResultSet rs = daoLayerObject.getPriceOfVechile(vechileType);
		while (rs.next()) {
			priceOfVechile.add(new Price(rs.getInt("id"), rs.getString("Type"),
					rs.getInt("DAILY"), rs.getInt("Monthly"), rs
							.getInt("Yearly")));
		}
	...
	public int createAGatePAss(int price, int vid, int uid) throws SQLException {
		return daoLayerObject.createAGatePAss(price, vid, uid);
	}
	...
	public ArrayList<User> getUserData(String userID) throws SQLException {
		...
		ResultSet rs = daoLayerObject.getUserData(userID);
		while (rs.next()) {
			alluserData.add(new User(rs.getInt("id"),
					rs.getString("full_name"), rs.getString("Password"), rs
	...
	public int updateUserData(String name, String password, String contact,
		...
		return daoLayerObject.UpdateUserData(name, password, contact, userID);
	}
}
