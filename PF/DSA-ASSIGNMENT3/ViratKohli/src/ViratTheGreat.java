 ...
 */
public class ViratTheGreat {

	private Bowller[] Heap;
	private int size;
	static int index = 0;

	/**
	 * It is a constructor to initialize heap
	 * @param maxSize maximum size of heap.
	 */
	public ViratTheGreat(int maxSize) {		
		this.size = -1;
		Heap = new Bowller[maxSize];
	}
	...
	/**
	 * It is a method used to return right child position.
	 ...
	 * @param ballsArray
	 */
	public void insert(Bowller ballsArray[]) {
		for (int i = 0; i < ballsArray.length; i++) {
			Heap[++size] = ballsArray[i];
			int current = size;
			while (Heap[current].getNumberOfBalls() > Heap[parent(current)].getNumberOfBalls()) {
	...
	private void maxHeapify(int pos) {
		...
		if (Heap[pos].getNumberOfBalls() < Heap[leftChild(pos)].getNumberOfBalls() || Heap[pos].getNumberOfBalls() < Heap[rightChild(pos)].getNumberOfBalls()) {

			if (Heap[leftChild(pos)].getNumberOfBalls() > Heap[rightChild(pos)].getNumberOfBalls()) {
	 ...
	 */
	public int[] extractMax(int numberOfBalls) {
		
		int bowlerSeq[] = new int[numberOfBalls];
	
		while (size != -1) {
			bowlerSeq[index] = Heap[0].getId();
			index++;
